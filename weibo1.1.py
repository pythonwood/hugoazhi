...
class Weibo:
    ...
    def login(self):
        ...
        data = json.loads(self.urlopen(url).read()[35:-1])
        ...
        o = self.urlopen(re.search('location\.replace\("(.*)"\);', o.read()).group(1))
...
没有进行request.close()操作；也可能是没有sleep几秒，导致网站认定这种行为是攻击。
        ...
        if type(user) is str:
            nickname = str(user)
            h = self.urlopen('http://weibo.com/'+nickname).read()
...
profile_ftype=1&key_word=%s&is_search=1&__rnd=1392289857878' % ('%s', '%s', '%s', uid, uid, '%s') # not kw hear
        ...
        # 重构了这段代码。好多了！ 2014-03-20 (论pytohn循环体后面的else的作用！) bug [Errno 10054]
        dl = []
        n = 1
        while(True):
    ...
    def pull_comment(self,postid):
        ...
        while(True):
            try:
                j = json.loads(self.urlopen(url % pagenum).read())
                d = j['data']
            ...
            count += len(m)
            d = d['page']
            if d['pagenum'] == d['totalpage'] or pagenum >= 500:  # 设置上限500面
                break
            pagenum += 1
...
$('a[usercard]').each(function(){this.href='http://weibo.com'+this.href.substring(this.href.lastIndexOf('/'))})
        ...
        for i in dl:
            ...
            # windows 下文件名不可以有:
            filename = dirname+i[0].replace(':','-')+'.html'
            filename = filename.decode('utf-8')
            if os.path.isfile(filename):
                continue
            sys.stdout.write( filename )

            data = self.pull_comment(i[1])

            for j in range(len(data)):
                data[j] = '%s.%s' % (''.join(data[j]),j+1)
            html = '<li>%s</li>' % ('</li>\n<li>'.join(data))
            html = tpl % (i[0], html)
            with open(filename, 'w') as f:
                f.write(html)
            ...
            print "\ndone(%skb:%s秒)" % (os.path.getsize(filename)/1024, round(time_used, 1))

    def pull_forward(self, id, filename):
        '''将id对应微博的转发下载到本地filename文件中，并简化美化。'''
...
if __name__ == '__main__':
    ...
    wb = Weibo('582223837@qq.com', 'wws1992', debug=True)
