...
Weibo.pull_comment(self,postid):
    将id对应微博的评论下载到本地filename文件中，并简化美化。

Weibo.pull_comment_worker(self, filename, dirname='weibo-data/'):
    下载符合格式的文件中的每条记录。

Weibo.pull_forward(self, postid):
    将id对应微博的转发下载到本地filename文件中，并简化美化。

Weibo.pull_forward_worker(self, filename, dirname='weibo-data/'):
    下载符合格式的文件中的每条记录。
